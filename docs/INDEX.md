...
├── dims.py         # 维度事件层：RSS 抓取 + HN/Reddit 热度 + LLM 故障转移链打标/抽词（1258 行）
...
| `dims.py` | 1258 | RSS 事件层 + LLM 故障转移链打标/抽词 | `get_dims`, `get_news_cards`, `start_background_dims_refresher`, `enrich_with_signals`, `_llm_classify_batch` | config, requests, terms |
