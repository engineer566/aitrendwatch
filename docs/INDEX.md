...
```
aitrendwatch/
├── app.py          # Flask 入口 + 路由 + 8 个直连抓取源、词详情装配（1529 行）
...
│   ├── index.html         # 首页主单页（1443 行：词卡/逐条新闻双视图，JS fetch + i18n，header 含 🤗 HF 入口）
...
|------|------|------|---------------------------------------|------|
| `app.py` | 1529 | Flask 入口、路由、8 直连源抓取、词详情装配 | 39 个路由 view 函数 + `_word_detail` + `_explain_fallback` + `_hf_models_for` | tracker, dims, terms, config, store, stream_utils |
