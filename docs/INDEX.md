...
├── config.py       # 全部配置/环境变量/降级开关 + LLM 提供方切换（121 行）
├── dims.py         # 维度事件层：RSS 抓取 + HN/Reddit 热度 + LLM(DeepSeek/GLM) 打标/抽词（1200 行）
...
| `config.py` | 121 | 配置集中地 + LLM 提供方切换 + `ensure_data_dir()` | `ensure_data_dir` | os |
| `dims.py` | 1200 | RSS 事件层 + LLM(DeepSeek/GLM) 打标/抽词 | `get_dims`, `get_news_cards`, `start_background_dims_refresher`, `enrich_with_signals`, `_llm_classify_batch` | config, requests, terms |
