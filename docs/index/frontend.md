...
| 🤗 HF 入口（header 右侧 `#hf-link`） | 355 | 与语言/主题按钮同排的 `.btn` 链接；href/title 由前端 `updateHfLink()`（591）跟随 `LANG` 动态生成，语言切换即时同步 |
...
## templates/hf.html  （361 行）— HuggingFace 独立排序页（开源动向）
...
| SEO ld+json | 213 | ItemList（模型榜 Top-20，爬虫可见） |
...
## templates/terms.html  （376 行）— 服务条款页
...
## templates/term_detail.html  （315 行）— 通用热词聚合页
...
## templates/search.html  （505 行）— 搜索结果页（含热词命中）
