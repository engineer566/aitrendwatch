...
## templates/index.html  （1331 行）— 首页主单页（词视图为主）
...
| ├ 返回滚动恢复 | 1210–1249 | 点击 `/term/` 链接前记 `window.scrollY` 到 sessionStorage（`aitw_last_scroll`）；后退/前进（back_forward）或经词条页「返回首页」链接（`scroll_back=1`）回首页时，在 SSR + `/api/stream` 异步渲染完成后恢复滚动并删除 key + 清理 URL 标记 |
| └ Mock 数据 | 1256 | 后端不可用时的内置预览数据 |
