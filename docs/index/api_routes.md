...
> 全部 41 条路由（含 errorhandler），按功能分组。每条带 `app.py:行号` 便于跳读。
...
| 路径 | 方法 | 函数 | 行号 | 功能 | 备注 |
|------|------|------|------|------|------|
| `/` | GET | `index` | `app.py:655` | 首页主单页（词视图为主 + 逐条新闻 tab） | 记 PV/visit/曝光，SSR 首批词卡，前端 JS 再拉 `/api/stream` |
| `/hf` | GET | `hf_page` | `app.py:944` | **HuggingFace 独立排序页（开源动向）**：按趋势分/点赞/下载量排序，每卡带 pipeline_tag 主徽标 + tags/官方/社区/论文 | 服务端渲染双语（zh/en），排序/语言切换零 fetch；数据复用 tracker 缓存 |
| `/term/<path:term_name>` | GET | `term_detail` | `app.py:746` | 通用热词聚合页（SEO 长尾）：任何词有页——相关报道聚合 + 词热度；HF 词额外含官方/社区/arXiv 区块 | 进程内 TTL 缓存；未找到 → 404 |
| `/terms` | GET | `terms` | `app.py:802` | 服务条款页（中英双语） | 静态文案，`SITE_TERMS_UPDATED` 常量 |
| `/admin/login` | GET,POST | `admin_login` | `app.py:1437` | 管理员登录 | `ADMIN_TOKEN` 未设 → 404 隐身 |
| `/admin` | GET | `admin_home` | `app.py:1460` | 赞助位管理后台 | 需 admin |
| `/monitor` | GET | `monitor` | `app.py:1502` | 流量监控页 | 需 admin，只读 PV/UV/地域 + 事件统计 |
...
| 路径 | 方法 | 函数 | 行号 | 功能 |
|------|------|------|------|------|
| `/health` | GET | `health` | `app.py:1013` | 健康检查 |
| `/api/click/<path:slot_id>` | GET | `sponsor_click` | `app.py:1408` | 赞助位点击计数 + 302 跳转 |
| `/admin/stats` | GET | `admin_stats` | `app.py:1495` | 赞助位 30 天统计（需 admin） |
| `/api/event` | POST | `api_event` | `app.py:1556` | 前端用户行为事件批量上报（JSON body） |
| `/monitor/api` | GET | `monitor_api` | `app.py:1510` | 监控页数据（`?days=1..90`，需 admin） |
| `/monitor/api/events` | GET | `monitor_events_api` | `app.py:1593` | 监控页用户事件统计（`?days=1..90`，需 admin） |

### Admin 写操作（需 admin，POST）

| 路径 | 方法 | 函数 | 行号 | 功能 |
|------|------|------|------|------|
| `/admin/sponsors` | POST | `admin_upsert_sponsor` | `app.py:1467` | 新建/更新赞助位 |
| `/admin/sponsors/<slot_id>/toggle` | POST | `admin_toggle_sponsor` | `app.py:1477` | 上下架切换 |
| `/admin/sponsors/<slot_id>/delete` | POST | `admin_delete_sponsor` | `app.py:1486` | 删除赞助位 |
...
- `admin_required` 装饰器（`app.py:1418`）：`ADMIN_TOKEN` 未设 → 所有 `/admin/*` 返回 404（隐身）。
