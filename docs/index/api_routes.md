...
| 路径 | 方法 | 函数 | 行号 | 功能 |
|------|------|------|------|------|
| `/robots.txt` | GET | `robots` | `app.py:1258` | 爬虫规则（SEO 关 → 禁止索引） |
| `/sitemap.xml` | GET | `sitemap` | `app.py:1278` | 站点地图（首页 + 热词详情页，上限 `SITEMAP_MAX_URLS`） |
| `/favicon.ico` | GET | `favicon` | `app.py:1316` | favicon |
| `/favicon.svg` | GET | `favicon_svg` | `app.py:1325` | favicon SVG |
| `/apple-touch-icon.png` | GET | `apple_touch_icon` | `app.py:1337` | Apple 触屏图标 |

## 错误处理

| 类型 | 函数 | 行号 | 功能 |
|------|------|------|------|
| 404 | `not_found` | `app.py:755` | 简单 HTML + `noindex`，防爬虫索引不存在的 term 页 |
