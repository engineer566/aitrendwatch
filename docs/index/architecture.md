...
### 2. 统一卡片流 `/api/stream`  （`app.py:1072`）  ← 前端主数据源
```
...
### 3. 单词聚合 `/api/word/<term>`  （`app.py:1243`）  ← 词卡展开「更多」+ 详情页共用
```
GET /api/word/<term>
...
### 4. 热词详情 `/term/<name>`  （`app.py:920`）  ← SEO 长尾 + 慢路径
```
GET /term/<name>
...
### 5. HuggingFace 排序页 `/hf` + `/api/hf`  （`app.py:1179` / `app.py:1217`）  ← 开源动向独立页
```
GET /hf?sort=trending|likes|downloads&lang=zh|en
  → _request_lang() 决定语言
  → _hf_models_for(sort, lang)  # app.py:1154
...
### 启动时机
`app.py:48-50` 模块加载时即 `start_background_refresher()` + `start_background_dims_refresher()`。每个 worker 进程各起线程，靠 fcntl 锁去重。

## 缓存层级（三级）

| 层级 | 介质 | 作用域 | TTL | 典型键 | 代码位置 |
|------|------|--------|-----|--------|----------|
| L1 内存 | 进程内 `dict` | 单 worker 进程 | 300s（单源）/ 1800s（详情） | `{source: (ts,data)}` | `app.py:64` `_cache` |
...
- 本地：`python app.py` 单进程 debug 模式，`app.py:3049` `app.run(host="0.0.0.0", port=5000, debug=True)`。
