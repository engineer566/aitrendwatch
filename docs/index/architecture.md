...
### 2. 统一卡片流 `/api/stream`  （`app.py:687`）  ← 前端主数据源
```
GET /api/stream?lang=zh&sort=rise&view=words
  → detect_region() 决定默认 lang
  → view=words → terms.get_word_cards(sort, lang)   # 读 cache/words.json（秒回）
  → view=news → tracker.get_model_cards + dims.get_news_cards  # 旧逻辑零回归
  → 按 sort(rise/hot/new) 排序（words 视图 new=新奇度）
  → {ok, view, fetched_at, count, dimension_list, terms}
```
**核心特征：请求路径零上游 IO**。词卡/新闻卡都来自后台预热线程写好的文件缓存。

### 3. 单词聚合 `/api/word/<term>`  （`app.py:744`）  ← 词卡展开「更多」+ 详情页共用
```
GET /api/word/<term>
...
### 4. 热词详情 `/term/<name>`  （`app.py:615`）  ← SEO 长尾 + 慢路径
```
GET /term/<name>
...
### dims 层  （`dims.py:1258` `start_background_dims_refresher`）
- 独立 daemon 线程，独立跨进程锁 `cache/.dims.refresh.lock`。
- `_dims_refresh_once` (`dims.py:1165`)：拉 17 个 RSS 源 → HN/Reddit 复合热度 → LLM 批量打标（故障转移链）+ 抽关键词（无 key 走降级：词典匹配抽词）→ 写 `cache/dims.json` + `news_store.upsert_cards` 入历史库 → **拿到锁的 worker 再调 `terms.refresh_words` 归并热词池 + 三榜打分 + 周期快照，写 `cache/words.json`**。
- **定点刷新**（Asia/Shanghai）：`DIMS_REFRESH_HOURS = (13,19,1,7)`（`config.py:86`），一天 4 次，6 小时一档。选点避开 DeepSeek 高峰段 + 命中硬盘缓存 TTL。`_seconds_until_next_refresh_hour` (`dims.py:1211`) 算下次刷新倒计时。
- `_persist_to_history` (`dims.py:1146`)：每轮把 cards 持久化到 `news.db`，供 `list_history_cards` 扩大内容池 + `terms` 词聚合扫描。
...
| L2 文件 | `cache/*.json` | 跨 worker 共享 | 后台线程刷新频率决定 | `terms.json`, `dims.json`, `words.json` | `tracker.py:62` / `dims.py:74` / `terms.py:38` |
