...
### 2. 统一卡片流 `/api/stream`  （`app.py:733`）  ← 前端主数据源
```
...
### 3. 单词聚合 `/api/word/<term>`  （`app.py:804`）  ← 词卡展开「更多」+ 详情页共用
```
GET /api/word/<term>
...
### 4. 热词详情 `/term/<name>`  （`app.py:658`）  ← SEO 长尾 + 慢路径
```
GET /term/<name>
...
| L2 文件 | `cache/*.json` | 跨 worker 共享 | 后台线程刷新频率决定 | `terms.json`, `dims.json`, `words.json` | `tracker.py:62` / `dims.py:76` / `terms.py:91` |
...
- 本地：`python app.py` 单进程 debug 模式，`app.py:1364` `app.run(port=5000, debug=True)`。
