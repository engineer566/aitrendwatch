...
## app.py  （1353 行）— Flask 入口 + 路由 + 直连抓取
...
## terms.py  （1248 行）— 词粒度聚合层（词维度重构，新增）

### 分区清单
| 行号范围 | 分区 |
|----------|------|
| 42–148 | 词池规模控制 + words.json 文件缓存（复刻 dims.py） |
| 150–196 | SQLite `init_db`：`terms` / `term_snapshots` 表 + WAL |
| 204–336 | 关键词词典 `_LEXICON`/`_ALIAS`/`_ASCII_PATTERNS`（版本感知词边界） |
| 338–576 | `normalize_term` / `extract_keywords_dict` / 标题关联匹配辅助（含 `_title_key` 标题归一化去重键，447） |
| 578–983 | 词聚合 + 三榜打分 + 快照（`refresh_words` / `_refresh_words_inner`，dims 刷新锁内调；top news 排序截断前按标题去重） |
| 986–1159 | 读：`get_word_cards` / `get_term_row` / `get_term_news` / `list_terms_for_sitemap`（`get_term_news` limit 截断前按标题去重） |
| 1161–1248 | 历史回填 `backfill_history` + CLI |

### 公开函数（被 app.py / dims.py 调用）
| 函数 | 行号 | 职责 |
|------|------|------|
| `init_db()` | 151 | 建 `terms`/`term_snapshots` 表 + WAL（失败 `_DB_OK=False`） |
| `normalize_term(s)` | 338 | 任意词形 → canonical 键（小写/别名/去复数） |
| `extract_keywords_dict(title)` | 362 | 词典匹配抽词（无 LLM key 降级 + 回填） |
| `refresh_words(all_cards, model_cards)` | 578 | 词池归并 + 热度/上升/新奇度打分 + 快照 + 写 words.json |
| `get_word_cards(sort, lang, limit)` | 986 | `/api/stream?view=words` 数据源（读 words.json，先完整排序再截取再投影） |
| `get_term_row(term)` | 1032 | 查 terms 主表（canonical 键） |
| `get_term_news(term, limit, lang)` | 1048 | 词 → 关联报道（canonical/别名 + 标题边界兜底；limit 截断前按归一化标题去重，同标题转载只留 score 最高者） |
| `list_terms_for_sitemap(limit)` | 1145 | sitemap 词表（热度降序） |
| `backfill_history(days, force)` | 1161 | 词典回填 keywords + 合成历史快照（幂等，--force 全量） |
