...
def build_display_overrides_block(config):
    """生成 _display_of 函数中的 overrides 和 lexicon_display 逻辑。"""
    ...
    if lexicon_display:
        ...
        items = [f'        "{k}": "{v}"' for k, v in sorted(lexicon_display.items())]
        lines.append(",\n".join(items))
        lines.extend([
            "    }",
...
def sync_terms_py(config, check_only=False):
    ...
    # 2. 替换 _display_of 中的 overrides + lexicon_display 区域
    overrides_block = build_display_overrides_block(config)
    if overrides_block:
        # 查找 _display_of 函数内的插入点（在 _upper_vals 检查之后，词典美化之前）
        ...
        if match:
            # 保留原有的 _upper_vals 检查(group(2))，在其后插入 overrides + lexicon_display
            new_block = match.group(1) + match.group(2) + "\n" + overrides_block + "\n" + match.group(3)
            content = content[:match.start()] + new_block + content[match.end():]
        else:
            print(f"[警告] 未找到 _display_of 插入点，跳过 display_overrides/lexicon_display 同步")
    ...
    print(f"[成功] 已同步 {len(config.get('lexicon_display', {}))} 个 lexicon_display")
